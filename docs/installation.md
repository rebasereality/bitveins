...
3. creates a random 48-byte session secret;
4. stores only the Scrypt password hash;
5. writes `~/.config/bitveins/env` with permissions `0600`;
6. installs and enables `~/.config/systemd/user/bitveins.service`;
7. binds the server to `127.0.0.1`;
8. waits for an HTTP 200 health response.
