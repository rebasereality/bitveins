...
test('keeps desktop and mobile appearance profiles independent without remounting the terminal', async ({ page }) => {
  ...
  try {
    ...
    await expect(page.getByRole('heading', { name: 'Agent Inbox notifications' })).toHaveCount(0)
    ...
    await expect(hermesSettings.getByRole('switch', { name: 'Input requests' })).toBeChecked()
    await expect(hermesSettings.getByRole('switch', { name: 'Permission requests', exact: true })).toBeChecked()
    await expect(hermesSettings.getByRole('switch', { name: 'Completed tool turns', exact: true })).toBeChecked()
    await expect(hermesSettings.getByRole('switch', { name: 'Failed turns', exact: true })).toBeChecked()
    const textOnlyResponses = hermesSettings.getByRole('switch', { name: 'Text-only responses', exact: true })
